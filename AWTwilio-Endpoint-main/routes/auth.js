 ...
 * POST /auth/request-delete-otp — send deletion OTP to authenticated user
 * DELETE /auth/account          — delete account (requires OTP code in body)
 */
    ...
    try {
        ...
        if (user) {
            if (user.verified) {
                ...
                // Account exists but is unverified. Auto-resend OTP and prompt frontend to verify.
                try {
                    const limit = await checkVerificationResendLimit(normalizedEmail);
                    if (limit.allowed) {
                        const code = await createOTP('verify', normalizedEmail);
                        await sendVerificationEmail(normalizedEmail, code);
                    }
                } catch (e) {
                    console.error('[auth/register] Failed to resend OTP for unverified user:', e);
                }
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user.verified) {
            // Automatically resend OTP and return 403 to trigger OTP screen on frontend
            try {
                const limit = await checkVerificationResendLimit(normalizedEmail);
                if (limit.allowed) {
                    const code = await createOTP('verify', normalizedEmail);
                ...
                console.error('[auth/login] Failed to send OTP for unverified user:', e);
            }
            return res.status(403).json({ 
...
router.post('/request-delete-otp', requireAuth, async (req, res) => {
    try {
        ...
        const code = await createOTP('delete', user.email);
        await sendVerificationEmail(user.email, code);

        res.json({ message: 'Deletion code sent to your email' });
    } catch (err) {
        console.error('[request-delete-otp]', err);
...
router.delete('/account', requireAuth, async (req, res) => {
    try {
        ...
        const otpValid = await verifyOTP('delete', user.email, code);
        if (!otpValid) {
